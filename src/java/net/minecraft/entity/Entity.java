...
import shop.xiaoda.event.world.EventStep;
import shop.xiaoda.event.world.EventStrafe;
import shop.xiaoda.module.modules.combat.AutoClicker;
...
public abstract class Entity implements ICommandSender
{
    ...
    public double lastGroundX;
    public double lastGroundY;
    public double lastGroundZ;
    public double lastLastGroundX;
    public double lastLastGroundY;
    public double lastLastGroundZ;
    ...
    public void moveEntity(double x, double y, double z)
    {
        if (this.noClip) {
            ...
            double d4 = y;
            double d5 = z;
            boolean ff = this.onGround && (this.isSneaking() || this.safeWalk) && this instanceof EntityPlayer;
            if (ff) {
                double d6 = 0.05;
            ...
            EventStep eventStep = new EventStep(this.stepHeight, true);
            if (this instanceof EntityPlayerSP) {
                EventManager.call(eventStep);
            }
            if (eventStep.getStepHeight() > 0.0 && flag1 && (d3 != x || d5 != z)) {
                double d11 = x;
                double d7 = y;
                double d8 = z;
            ...
            boolean bl = this.isCollided = this.isCollidedHorizontally || this.isCollidedVertically;
            if (this.onGround) {
                this.lastLastGroundX = this.lastGroundX;
                this.lastLastGroundY = this.lastGroundY;
                this.lastLastGroundZ = this.lastGroundZ;
                this.lastGroundX = this.posX;
                this.lastGroundY = this.posY;
                this.lastGroundZ = this.posZ;
            }
            if ((block1 = this.worldObj.getBlockState(blockpos = new BlockPos(i = MathHelper.floor_double(this.posX), j = MathHelper.floor_double(this.posY - (double)0.2f), k = MathHelper.floor_double(this.posZ))).getBlock()).getMaterial() == Material.air && ((block = this.worldObj.getBlockState(blockpos.down()).getBlock()) instanceof BlockFence || block instanceof BlockWall || block instanceof BlockFenceGate)) {
                block1 = block;
            ...
            if (this.canTriggerWalking() && !ff && this.ridingEntity == null) {
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        ...
        if (f >= 1.0E-4f) {
            ...
            if (Client.instance.moduleManager.getModule(AutoClicker.class).state && (Double)AutoClicker.jitter.getValue() != 0.0 && Mouse.isButtonDown(0)) {
                f1 = MathHelper.sin((this.rotationYaw + (float)(AutoClicker.b ? AutoClicker.c : -AutoClicker.c)) * (float)Math.PI / 180.0f);
                f2 = MathHelper.cos((this.rotationYaw + (float)(AutoClicker.b ? AutoClicker.c : -AutoClicker.c)) * (float)Math.PI / 180.0f);
            }
    ...
    public double getDistance(BlockPos pos) {
        double x = this.posX - (double)pos.getX();
        double y = this.posY - (double)pos.getY();
        double z = this.posZ - (double)pos.getZ();
