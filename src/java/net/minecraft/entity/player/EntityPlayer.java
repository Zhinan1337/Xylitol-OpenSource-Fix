...
import shop.xiaoda.utils.waveycapes.sim.StickSimulation;
...
public abstract class EntityPlayer extends EntityLivingBase
{
    ...
    public void updateSimulation(final EntityPlayer abstractClientPlayer, final int partCount) {
        boolean dirty = false;
        if (this.stickSimulation.points.size() != partCount) {
            ...
            for (int i = 0; i < partCount; ++i) {
                ...
                if (i > 0) {
                    this.stickSimulation.sticks.add(new StickSimulation.Stick((StickSimulation.Point)this.stickSimulation.points.get(i - 1), point, 1.0f));
                }
            }
            dirty = true;
        }
    ...
    public void simulate(final EntityPlayer abstractClientPlayer) {
        if (this.stickSimulation.points.isEmpty()) {
            return;
        }
        this.stickSimulation.points.get(0).prevPosition.copy(this.stickSimulation.points.get(0).position);
        ...
        final float heightMul = 6.0f;
        final double fallHack = MathHelper.clamp_double(this.stickSimulation.points.get(0).position.y - abstractClientPlayer.posY * heightMul, 0.0, 1.0);
        final StickSimulation.Vector2 position = this.stickSimulation.points.get(0).position;
        position.x += (float)(d * o + m * p + fallHack);
        this.stickSimulation.points.get(0).position.y = (float)(abstractClientPlayer.posY * heightMul + (abstractClientPlayer.isSneaking() ? -4 : 0));
    ...
    public void onDeath(DamageSource cause) {
        ...
        if (cause != null) {
            this.motionX = -MathHelper.cos((this.attackedAtYaw + this.movementYaw) * 3.1415927f / 180.0f) * 0.1f;
            this.motionZ = -MathHelper.sin((this.attackedAtYaw + this.movementYaw) * 3.1415927f / 180.0f) * 0.1f;
        } else {
            final double n = 0.0;
            this.motionZ = n;
            this.motionX = n;
    ...
    public void attackTargetEntityWithCurrentItem(Entity targetEntity)
    {
        if (targetEntity.canAttackWithItem() && !targetEntity.hitByEntity((Entity)this)) {
            ...
            if (f > 0.0f || f2 > 0.0f) {
                ...
                if (flag3) {
                    if (i > 0) {
                        targetEntity.addVelocity((double)(-MathHelper.sin(this.movementYaw * 3.1415927f / 180.0f) * i * 0.5f), 0.1, (double)(MathHelper.cos(this.movementYaw * 3.1415927f / 180.0f) * i * 0.5f));
                        final EventSlowDown event = new EventSlowDown(EventSlowDown.Type.Sprinting, 0.2f, 0.2f);
