...
import shop.xiaoda.event.rendering.EventRLE;
import shop.xiaoda.event.rendering.EventRenderModel;
import shop.xiaoda.module.modules.render.MoBendsMod;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T>
{
    ...
    public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks)
    {
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, new Object[] {entity, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z)}))
        {
            ...
            if (Client.instance.moduleManager.getModule(MoBendsMod.class).onRenderLivingEvent(this, entity, x, y, z, entityYaw, partialTicks)) {
                return;
            }
    ...
    protected void renderModel(final T entitylivingbaseIn, final float x, final float y, final float z, final float entityYaw, final float entityPitch, final float scaleFactor, final float partialTicks) {
        ...
        final boolean flag2 = !flag && !entitylivingbaseIn.isInvisibleToPlayer((EntityPlayer)Minecraft.getMinecraft().thePlayer);
        if (flag || flag2) {
            if (flag2) {
                GlStateManager.pushMatrix();
            ...
            GL11.glDisable(3008);
            final EventRenderModel renderModelEvent = new EventRenderModel(entitylivingbaseIn, () -> this.mainModel.render(entitylivingbaseIn, x, y, z, entityYaw, entityPitch, scaleFactor), () -> this.renderLayers(entitylivingbaseIn, x, y, partialTicks, z, entityYaw, entityPitch, scaleFactor));
