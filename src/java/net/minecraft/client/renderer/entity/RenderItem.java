...
public class RenderItem implements IResourceManagerReloadListener
{
    ...
    public void renderItemIntoGUI(final ItemStack stack, final int x, final int y, final float alpha) {
    ...
    public void renderItemIntoGUI(final ItemStack stack, final double x, final double y, final float alpha) {
        this.renderItemGui = true;
        final IBakedModel ibakedmodel = this.itemModelMesher.getItemModel(stack);
        GlStateManager.pushMatrix();
        this.textureManager.bindTexture(TextureMap.locationBlocksTexture);
    ...
    public void renderItemOverlayIntoGUI(final FontRenderer fr, final ItemStack stack, final double xPosition, final double yPosition, final String text) {
        if (stack != null) {
            ...
            if (stack.isItemDamaged()) {
                ...
                this.draw(worldrenderer, xPosition + 2.0, yPosition + 13.0, 13, 2, 0, 0, 0, 255);
                this.draw(worldrenderer, xPosition + 2.0, yPosition + 13.0, 12, 1, (255 - i) / 4, 64, 0, 255);
                int k = 255 - i;
                int l = i;
                int m = 0;
                if (Config.isCustomColors()) {
                ...
                this.draw(worldrenderer, xPosition + 2.0, yPosition + 13.0, j1, 1, k, l, m, 255);
                GlStateManager.enableBlend();
                GlStateManager.enableAlpha();
    ...
    private void draw(final WorldRenderer renderer, final double x, final double y, final int width, final int height, final int red, final int green, final int blue, final int alpha) {
